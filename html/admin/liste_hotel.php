...
try {
    ...
    // Fetch reservations
    $stmt = $dbpdo->prepare("SELECT id_reservation, utilisateur, hotel, nbr_personne, date, date_fin FROM reservation_hotel");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
  ...
  function supprimer_utilisateur($id){
    try {
        ...
        $requete = $dbpdo->prepare("DELETE FROM reservation_hotel WHERE id_reservation = :id");
        ...
        <main class="main-container">
            ...
            <table border="1">
                ...
                <?php foreach ($hotels as $contact): ?>
                    ...
                    $idd = $contact['hotel'];
                    $requete = $dbpdo->prepare("SELECT * FROM hotel WHERE id_hotel = :id");
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($contact['nbr_personne']) ?></td>
                        <td><?= htmlspecialchars($contact['date']) ?></td>
                        <td><?= htmlspecialchars($contact['date_fin']) ?></td>
                        <?php 
                        $id_reservation = $contact['id_reservation'] ;
